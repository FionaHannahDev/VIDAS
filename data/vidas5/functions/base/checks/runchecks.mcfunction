...
execute if score VIDAS.Config VSCCircles matches 1 as @s[tag=!VSReady] at @s if block ~ ~-1 ~ crying_obsidian run function vidas5:base/checks/circle
execute if score VIDAS.Config VSCFlash matches 1 as @s[tag=!VSReady] at @s if block ~ ~-1 ~ crying_obsidian run function vidas5:base/checks/flash

#If no crying obsidian, or no lockin, remove tag
execute as @s[tag=!VSLockin] at @s unless block ~ ~-1 ~ crying_obsidian run function vidas5:base/checks/removetptags
