...
scoreboard players remove VIDAS.Config VSCT10 1
...
execute if score VIDAS.Config VSCTtest <= VIDAS.Config VSCT9 run scoreboard players operation VIDAS.Config VSCTtest = VIDAS.Config VSCT8
...
execute if score VIDAS.Config VSCT10 <= VIDAS.Config VSCTtest run function vidas5:ui/loadscreen/config/timings/config/count/10/floor
